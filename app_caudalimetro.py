...
# 2. CSS MAESTRO (BLINDAJE DEL CENTRO)
st.markdown("""
    <style>
    ...
    /* Forzamos que el contenedor de contenido ignore el empuje del sidebar */
    ...
    /* Anulamos el padding lateral que Streamlit inyecta al abrir el sidebar */
    [data-testid="stMain"] {
    ...
    with placeholder.container():
        st.markdown(f"""
            <div class="loading-overlay">
                ...
                <p style="color:#00d4ff; font-weight:bold; margin-top:10px; font-size:1.5rem;">Analizando inducción...</p>
            </div>
        """, unsafe_allow_html=True)
        time.sleep(2.5)
    placeholder.empty()

    # Cálculo basado en la Ley de Faraday: V = B * D * v
    A_m2 = np.pi * (D_si / 2)**2
    v_range = np.linspace(0.1, 5.0, 100)
    # Factor de corrección por conductividad (simplificado para la simulación)
    f_cond = 1 / (1 + np.exp(-0.01 * (sigma_si - 5)))
    V_mv = (B_si * D_si * v_range * f_cond * 1000)
