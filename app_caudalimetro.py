...
# 2. CSS Maestro (Título fijo, botones opacos, diseño profesional)
st.markdown("""
    <style>
...
# Factor de Error
error_factor = st.slider('Ajuste del Factor de Error', 0.80, 1.20, 1.00, 0.01)
...
# --- 6. CALCULADORA INTERACTIVA CON SEÑALIZACIÓN ---
st.markdown(f"#### 🧮 Calculadora Predictiva y Gráfica")
c_col1, c_col2 = st.columns(2)

with c_col1:
    q_calc = st.number_input(f"Ingresa Caudal (Q) en {u_q}", value=0.0, format="%.4f")
    v_res = q_calc * m_eq
    st.success(f"Voltaje Resultante: **{v_res:.4f} mV**")

with c_col2:
    v_calc = st.number_input(f"Ingresa Voltaje (V) en mV", value=0.0, format="%.4f")
    q_res = v_calc / m_eq if m_eq != 0 else 0
    st.info(f"Caudal Resultante: **{q_res:.4f} {u_q}**")

# --- 7. GENERACIÓN DE GRÁFICA CON PUNTO SEÑALIZADO ---
# Definimos el rango de la gráfica basado en la entrada del usuario para que siempre sea visible
max_q = max(q_calc, q_res, 1.0) * 1.2
q_range = np.linspace(0, max_q, 100)
v_range = q_range * m_eq



plt.style.use('dark_background')
fig, ax = plt.subplots(figsize=(10, 5))
ax.plot(q_range, v_range, color='#00d4ff', linewidth=2, label='Curva de Calibración')

# Señalización del punto (Si hay valores ingresados)
target_q = q_calc if q_calc > 0 else q_res
target_v = v_res if q_calc > 0 else v_calc

if target_q > 0:
    ax.scatter(target_q, target_v, color='#ff4b4b', s=100, zorder=5, label='Punto Calculado')
    ax.axhline(target_v, color='white', linestyle='--', alpha=0.3)
    ax.axvline(target_q, color='white', linestyle='--', alpha=0.3)
    ax.annotate(f'({target_q:.2f}, {target_v:.2f})', (target_q, target_v), 
                textcoords="offset points", xytext=(10,10), ha='center', color='#ff4b4b')
...
st.write("---")
st.caption("Adriana Teixeira Mendoza 2026")import streamlit as st
import numpy as np
import matplotlib.pyplot as plt
