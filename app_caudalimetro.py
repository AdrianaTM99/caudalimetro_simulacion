...
# =====================================================
# SIDEBAR FLOTANTE CON EXPANDERS VISIBLES
# =====================================================
...
# =====================================================
# CSS MAESTRO ORIGINAL (SIN MODIFICACIONES)
# =====================================================
...
with col1:
    ...
    B_user = st.slider(f'Ajustar B', float(b_min), float(b_max), float(B_val))

with col2:
    sig_val = st.number_input(f'σ: Conductividad ({u_sig})', float(sig_min), float(sig_max), float(sig_def))
    sigma_user = st.slider(f'Ajustar σ', float(sig_min), float(sig_max), float(sig_val))

with col3:
    D_val = st.number_input(f'D: Diámetro ({u_d})', float(d_min), float(d_max), float(d_def), format="%.4f")
    D_user = st.slider(f'Ajustar D', float(d_min), float(d_max), float(D_val))
